...
def games_show_players(update: Update, context: CallbackContext) -> int:
    ...
    for player in players:
        if player[1] == 0:
            counter += 1
            players_msg += '\n' + str(counter) + '. ' + player[0]
        if player[1] == 1:
            reserve_counter += 1
            players_msg += '\n' + 'Р' + str(reserve_counter) + '. ' + player[0]
...
# =============================================================================MAIN=====================================
ASKED_TO_STORE_VDATA, RECORD_NAME, ASKED_DATE, ASKED_PLACE, ASKED_PLAYERS, ASKED_DESCRIPTION, ASKED_TO_CHECK, \
    ASKED_GAME, ASKED_PLAYERS_LIST, ASKED_GAME_TO_LEAVE, ASKED_ABOUT_RESERVE = range(11)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        entry_points=[MessageHandler(Filters.regex(r'Create game'), cr_game), CommandHandler('start', start),
        ...
        states={
            ASKED_DATE: [
                MessageHandler(base_filter, cr_get_date)
            ],
            ASKED_PLACE: [
                MessageHandler(base_filter, cr_get_place)
            ],
            ASKED_PLAYERS: [
                MessageHandler(base_filter, cr_get_players_num)
            ],
            ASKED_DESCRIPTION: [
                MessageHandler(base_filter, cr_get_description)
            ],
            ASKED_TO_CHECK: [
                MessageHandler(base_filter, cr_finish)
            ],
            ASKED_TO_STORE_VDATA: [
                MessageHandler(base_filter, vdata_ask_perm)
            ],
            RECORD_NAME: [
            ...
            ASKED_PLAYERS_LIST: [
                MessageHandler(base_filter, games_show_players)
            ],
            ASKED_GAME_TO_LEAVE: [
                MessageHandler(base_filter, leave_success)
            ],
            ASKED_ABOUT_RESERVE: [
                MessageHandler(base_filter, reg_added_to_reserve)
            ]
        },
